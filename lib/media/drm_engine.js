...
shaka.media.DrmEngine = class {
  ...
  prepareMediaKeyConfigsForVariants_(variants) {
    ...
    console.warn('prepareMediaKeyConfigsForVariants_', allDrmInfo);
        ...
        for (const info of stream.drmInfos) {
          ...
          if (info.initData && info.initData.length) {
            config.initDataTypes = [
              ...new Set(info.initData.map(({initDataType}) => initDataType)),
            ];
          } else {
          ...
          if (info.persistentStateNotAllowed) {
  ...
  async queryMediaKeys_(configsByKeySystem) {
    ...
    for (const shouldHaveLicenseServer of [true, false]) {
      for (const keySystem of configsByKeySystem.keys()) {
        ...
        console.warn('requestMediaKeySystemAccess', keySystem, config);
        ...
        try {
          ...
          console.warn('requestMediaKeySystemAccess', error);
        } // Suppress errors.
        this.destroyer_.ensureNotDestroyed();
      }
    ...
    try {
      ...
      console.warn('mediaKeySystemAccess realConfig', {realConfig});
  ...
  createTemporarySession_(initDataType, initData) {
    ...
    try {
      if (this.usePersistentLicenses_ ||
        ...
        console.warn('Creating new persistent session');
        session = this.mediaKeys_.createSession('persistent-license');
      } else {
        shaka.log.v1('Creating new temporary session');
        console.warn('Creating new temporary session');
  ...
  static createDrmInfoFor_(keySystem, config) {
    ...
    return {
      ...
      persistentStateNotAllowed: (config.persistentState == 'not-allowed'),
  ...
  static fillInDrmInfoDefaults_(drmInfo, servers, advancedConfigs) {
    ...
    if (advancedConfig) {
      ...
      if (!drmInfo.persistentStateNotAllowed) {
        drmInfo.persistentStateNotAllowed =
            advancedConfig.persistentStateNotAllowed;
      }
