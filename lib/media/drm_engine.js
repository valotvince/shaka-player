...
shaka.media.DrmEngine = class {
  ...
  constructor(playerInterface, updateExpirationTime = 1) {
    ...
    /** @private {string} */
    this.newSessionType_ = 'temporary';
  ...
  prepareMediaKeyConfigsForVariants_(variants) {
    ...
    const advancedConfigs = shaka.util.MapUtils.asMap(
    ...
    for (const variant of variants) {
      ...
      for (const stream of [audio, video]) {
        ...
        for (const info of stream.drmInfos) {
          ...
          const advancedConfig = advancedConfigs.get(info.keySystem);

          if (advancedConfig && advancedConfig.sessionType) {
            config.sessionTypes = [advancedConfig.sessionType];
          }
        ...
        try {
          if (keySystem == 'com.microsoft.playready' &&
            ...
            try {
              ...
              this.newSessionType_ = config.sessionTypes[0];
              break;
            } catch (error) {
              shaka.log.v2(
  ...
  createTemporarySession_(initDataType, initData) {
    ...
    try {
      shaka.log.info('Creating new', this.newSessionType_, 'session');

      session = this.mediaKeys_.createSession(this.newSessionType_);
    } catch (exception) {
      this.onError_(new shaka.util.Error(
          shaka.util.Error.Severity.CRITICAL,
          shaka.util.Error.Category.DRM,
